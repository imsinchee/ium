...
var db = firebase.database();
    ...
    getharvest.on('value', function(snapshot){
        har=snapshot.val()
        if (har < 0){
            document.getElementById("harvest").innerHTML=String(getRandomInt(60,90))+"%";
        }
        else{
    ...
    getday.on('value', function(snapshot){
        days=snapshot.val()
        if (har < 0){
            document.getElementById("days").innerHTML=String(getRandomInt(30,90))+" days";
        }
        else{
            document.getElementById("days").innerHTML=String(har)+" days";
        }
    })
